...
export interface PotentialEncounter {
  type: 'combat' | 'social' | 'trap' | 'event';
  participants: Array<TRPGCharacter | NPCCharacter | EnemyCharacter>;
  location: Base;
...
export class EncounterDetectionSystem {
  /**
   * 空間的衝突をチェック - 同じ場所にいるキャラクター/エネミーを検出
   */
  static checkSpatialCollision(context: EncounterContext): PotentialEncounter[] {
    const encounters: PotentialEncounter[] = [];
   ...
   */
  static checkTemporalOverlap(context: EncounterContext): PotentialEncounter[] {
    const encounters: PotentialEncounter[] = [];
    ...
    const activeEvents = events.filter(event => 
      event.day === time.day &&
      event.timeOfDay === time.timeOfDay &&
    ...
    activeEvents.forEach(event => {
      if (event.type === 'trap') {
        encounters.push({
          ...
          participants: [],
          location,
          triggerConditions: ['時限式罠', 'イベントトリガー'],
          priority: 'high',
          aiRecommendedAction: '罠発動。反射神経判定を要求。',
        });
   ...
   * サプライズ判定が必要かチェック
   */
  static calculateSurpriseConditions(
    encounter: PotentialEncounter,
    ...
    const ambushEnemies = encounter.participants.filter(
      p => 'enemyType' in p && (p.tactics?.includes('ambush') || p.tactics?.includes('stealth'))
    );
   ...
   * AI戦術判断を生成
   */
  static generateTacticalJudgment(
    encounter: PotentialEncounter,
    context: EncounterContext
  ): AITacticalJudgment {
   ...
   * 遭遇の優先度を計算
   */
  static prioritizeEncounters(encounters: PotentialEncounter[]): PotentialEncounter[] {
   ...
   * 総合的な遭遇検出
   */
  static detectEncounters(context: EncounterContext): {
