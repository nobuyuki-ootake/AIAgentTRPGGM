...
(async () => {
  console.log('🔄 Testing Developer Mode with Fresh Server on Port 5174...');
  ...
  try {
    ...
    await page.screenshot({ 
      path: './test-results/fresh-5174-initial.png',
      fullPage: true 
    });
    
    // Look for developer toggle by data-testid first (safer)
    console.log('🎯 Looking for developer toggle by data-testid...');
    const devToggleByTestId = await page.getByTestId('developer-toggle');
    const isVisible = await devToggleByTestId.isVisible();
    ...
    if (isVisible) {
      ...
      await page.screenshot({ 
        path: './test-results/fresh-5174-before-toggle.png',
        fullPage: true 
      });
      ...
      // Click the developer toggle by data-testid
      console.log('🔧 Clicking developer toggle by data-testid...');
      await devToggleByTestId.click();
      ...
      await page.screenshot({ 
        path: './test-results/fresh-5174-after-toggle.png',
        fullPage: true 
      });
      ...
      if ((isCurrentlyOff && isNowOn) || (!isCurrentlyOff && isNowOff)) {
        ...
        await page.screenshot({ 
          path: './test-results/fresh-5174-final.png',
          fullPage: true 
        });
        
        // Final developer mode ON test for potential errors
        if (finalState === 'OFF') {
      ...
      if (errors.length > 0) {
        ...
        console.log('✅ No errors detected - developer mode working perfectly on fresh server!');
        console.log('✅ Original setDefaultActions issue is completely resolved!');
      }
      
    } else {
      console.log('❌ Developer toggle with data-testid still not found');
    }
    
  } catch (error) {
    console.log(`💥 Test Error: ${error.message}`);
    await page.screenshot({ 
      path: './test-results/fresh-5174-error.png',
      fullPage: true 
    });
  }
  ...
  if (errors.length === 0) {
    console.log('✅ Developer mode toggle working correctly with fresh server on port 5174');
