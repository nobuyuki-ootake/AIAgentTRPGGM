...
- [ ] **ユーザー操作ワークフロー Mermaid 図作成**
  - [ ] 一般ユーザー: セッション開始からクリアまでのフロー図
  - [ ] 一般ユーザー: シナリオ作成・保存フロー図
  - [ ] 開発者ユーザー: キャラクター管理フロー図
  - [ ] 開発者ユーザー: タイムライン・イベント管理フロー図

**重要度：HIGH**

- [ ] **画面遷移図の詳細化**
  - [ ] モード切り替え（一般/開発者）による表示変化の図解
  - [ ] エラー状態・例外処理フローの可視化
  - [ ] AI 応答待機・非同期処理フローの図解
...
### 🧪 **Playwright E2E テスト実装詳細**

#### 🎯 **実装項目**

**重要度：CRITICAL**

- [ ] **一般ユーザー: TRPGセッション完了テスト**
  ...
  **具体的な確認項目:**
  - [ ] セッション開始から 1 つ以上のイベントを発生させる
  - [ ] タイムライン上で設定されたイベントが適切に起動する
  - [ ] 敵キャラクターとの接敵時に戦闘画面が表示される
  - [ ] トラップイベントが発生し、ダイス判定が要求される
  - [ ] AI GM の応答が適切に表示される
  - [ ] セッション進行状況が適切に保存される
  - [ ] セッション完了時に結果画面が表示される
...
- [ ] **開発者ユーザー: キャラクター管理テスト**
  ...
  **具体的な確認項目:**
  - [ ] 開発者モード切り替えによる UI 変化確認
  - [ ] PC キャラクターの新規作成フォーム動作確認
  - [ ] NPC キャラクターの追加・ステータス設定確認
  - [ ] エネミーキャラクターの戦闘データ設定確認
  - [ ] キャラクター画像アップロード機能確認
  - [ ] キャラクター一覧での表示・編集・削除確認
  - [ ] ゲームシステムテンプレート適用確認

- [ ] **開発者ユーザー: タイムラインイベント管理テスト**
  ...
  **具体的な確認項目:**
  - [ ] タイムラインページでのイベント一覧表示確認
  - [ ] 新規イベント作成フォームの入力検証確認
  - [ ] イベント発生条件（時間・場所・キャラクター）設定確認
  - [ ] トラップイベント・戦闘イベント・ストーリーイベントの設定確認
  - [ ] イベント発生確率・難易度設定確認
  - [ ] イベント間の依存関係設定確認
  - [ ] イベントプレビュー・テスト実行機能確認
...
- [ ] **総合ワークフローテスト**
  ...
  **具体的な確認項目:**
  - [ ] 開発者モードでキャンペーン作成 → 一般モードでプレイ の全体フロー
  - [ ] 複数キャラクター同時操作での整合性確認
  - [ ] ブラウザリロード・タブ切り替え時のセッション状態保持確認
  - [ ] エラー発生時の適切な回復処理確認

- [ ] **パフォーマンス・安定性テスト**
  
  **具体的な確認項目:**
  - [ ] 長時間セッション（2 時間以上）での安定性確認
  - [ ] 大量イベント（50 個以上）設定時のパフォーマンス確認
  - [ ] 大量キャラクター（20 体以上）管理時の動作確認
  - [ ] ネットワーク切断・復旧時の動作確認
...
- [ ] **テスト環境構築**
  - [ ] Playwright 設定ファイルの TRPG 特化カスタマイズ
  - [ ] テストデータの自動生成・クリーンアップ機構
  - [ ] 並列テスト実行時の状態分離確保
  - [ ] CI/CD パイプラインへの統合
...
- [ ] **テストユーティリティ開発**
  - [ ] TRPG セッション操作の共通ヘルパー関数群
  - [ ] モード切り替え・状態確認の汎用関数
  - [ ] AI 応答待機・非同期処理の安定した待機機構
  - [ ] スクリーンショット・動画記録による証跡保存
